...
**`triggerTribe` condition** (`CardEffect.condition?: { type: 'triggerTribe'; tribe: Tribe }`) restricts a Mourn/Muster effect to firing only when *the minion that died/was cast* — not this effect's own source minion — is of the given tribe; it's the only `EffectCondition` variant meaningful on just two triggers (`validateCardDefinition.ts` rejects it elsewhere). `TurnStateMachine.triggerBoardWide` threads an `eventMinionDefinitionId` (the dying/cast minion, passed by `sweepDeaths`/`executePlayCard`'s Muster branch only) down into `triggerEffects`, which checks it via `minionHasTribe` alongside the existing Momentum check. `ai/scoring.ts`'s `mournBoardValue`/`musterBoardValue` mirror this with `triggerTribeSatisfied`, given the dying minion's/played card's own `CardDefinition`. No new card-text flavor prefix like `Momentum(N):` — the restriction reads naturally into each trigger's existing phrasing instead (see "Card design conventions" below), e.g. "When one of your Demon minions dies, ..." / "When you cast a Demon minion, ...".
